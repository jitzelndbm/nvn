...
function Navigation:search_link(fallback, direction, filter)
	...
	status, found_or_err = xpcall(self.note.buf_call, err.handler, self.note, function()
		local tree = vim.treesitter.get_parser(0, 'markdown_inline'):parse()
		local root = tree[1]:root()
		local result = vim.treesitter.query.parse('markdown_inline', LINK_QUERY)
		...
		for j = start_index, end_index, step do
			...
			local success, link_or_err = xpcall(Link.new, err.handler, nodes[j])
			if not success then error("Construction of a link failed" .. link_or_err) end

			-- Edit first and last
			if not fallback_link then fallback_link = link_or_err end

			-- If premise is met, return the link
			if filter(row - 1, col, lrow, lcol) then
				return link_or_err
			end
		...
		if fallback then
			...
			error("No link could be found and fallback was disabled" .. found_or_err)
		end
	end)
...
function Navigation:current_link()
	...
	local link_creator = types[node:type()] or error("TSNode under cursor is not of right type")
